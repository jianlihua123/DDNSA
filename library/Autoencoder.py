...
class Autoencoder(object):
    ...
    def __init__(self, n_layers, transfer_function=tf.nn.sigmoid, optimizer=tf.train.AdamOptimizer(), ae_para=[0, 0], flag=0):
        ...
        #
        # W11 = self.weights['encode'][-1]['w']
        # W22 = self.weights['recon'][-1]['w']
        lamda = 0.003
        ...
        if self.sparse_reg == 0:
            ...
            if flag == 0:
                ...
                # w22 = self.weights['recon'][-1]['w']
                self.cost = 0.5 * tf.reduce_sum(tf.pow(tf.subtract(self.reconstruction, self.x), 2.0)) + \
                            self.sparse_reg * self.kl_divergence(self.sparsity_level, self.hidden_encode[-1]) #+0.5 * 0.003 * (tf.reduce_sum(w11 * w11) + tf.reduce_sum(w22 * w22))
            else:
                # w11 = self.weights['encode'][0]['w']
                # w22 = self.weights['recon'][0]['w']

                self.cost = 0.5 * tf.reduce_sum(tf.pow(tf.subtract(self.reconstruction, self.x), 2.0))#+0.5 * 0.003 * (tf.reduce_sum(w11 * w11) + tf.reduce_sum(w22 * w22))
                #self.cost = 0.5*tf.reduce_sum(tf.pow(tf.subtract(self.reconstruction, self.x), 2.0))+\
